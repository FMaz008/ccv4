	...
	public static function generatePage(&$tpl, &$session, &$account, &$mj)
	{
		...
		$query = 'UPDATE `' . DB_PREFIX . 'item_inv`'
					. ' SET `inv_persoid` 			= :inv_persoid,'
						...
						. ' `inv_nocompte` 			= :inv_nocompte,'
						. ' `inv_nobanque` 			= :inv_nobanque,'
						. ' `inv_nip` 				= :inv_nip'
					. ' WHERE `inv_id` = :inv_id;';
		...
		$nobanque = '';
		$nocompte = '';
		if(!empty($_POST['inv_nocompte']))
		{
			$compte_exp = explode( "-",$_POST['inv_nocompte']);
			$nobanque = $compte_exp[0];
			$nocompte = $compte_exp[1].'-'.$compte_exp[2].'-'.$compte_exp[3];
		}
		
		if($nocompte == '')
			$prep->bindValue(':inv_nocompte', NULL, PDO::PARAM_NULL);
		else
			$prep->bindValue(':inv_nocompte', $nocompte, PDO::PARAM_INT);
			
		if($nobanque == '')	
			$prep->bindValue(':inv_nobanque', NULL, PDO::PARAM_NULL);
		else
			$prep->bindValue(':inv_nobanque', $nobanque, PDO::PARAM_INT);
		
		if(empty($_POST['inv_nip']))
			$prep->bindValue(':inv_nip', NULL, PDO::PARAM_NULL);
		else
			$prep->bindValue(':inv_nip', $_POST['inv_nip'], PDO::PARAM_INT);
		
		$prep->bindValue(':inv_notel', $_POST['inv_notel'], PDO::PARAM_INT);
